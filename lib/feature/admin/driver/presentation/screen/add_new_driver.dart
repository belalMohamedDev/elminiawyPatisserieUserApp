...
class _AddNewDriverState extends State<AddNewDriver> {
  ...
  Widget build(BuildContext context) {
    final driverCubit = context.watch<DriverCubit>();
    ...
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text(
          "${context.translate(AppStrings.addNew)} ${context.translate(AppStrings.driver)}",
          style: Theme.of(
            context,
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              _textFormData(driverCubit, context, responsive),
  ...
  BlocConsumer<DriverCubit, DriverState> _createProductButton() {
    return BlocConsumer<DriverCubit, DriverState>(
      listener: (context, state) {
        if (state is GetAllDriverSuccess) {
          context.pop();
        } else if (state is GetAllDriverError) {
      ...
      children: [
        ...
        CustomDropdownButtonFormField(
          ...
          onChanged: (value) {
            ...
            context.read<DriverCubit>().setBranchAreaId(storeAreaId!);
          },
        ),
      ],
    );
...
class _AdvancedImagePickerState extends State<AdvancedImagePicker>
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        GestureDetector(
          onTap: () => context.read<DriverCubit>().pickImage(),
          child: DottedBorder(
            ...
            child: ClipRRect(
              ...
              child: Container(
                ...
                child: context.read<DriverCubit>().imageFile == null
                    ? _buildPlaceholder()
                    : _buildImagePreview(),
              ),
            ),
  ...
  Widget _buildImagePreview() {
    return Stack(
      fit: StackFit.expand,
      children: [
        Image.file(context.read<DriverCubit>().imageFile!, fit: BoxFit.cover),
        Container(
